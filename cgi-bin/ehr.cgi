...
my $ehr_file = $q->param("ehr-file");
my $ehr_file_fh = $q->upload("ehr-file");
my $ehr_text = $q->param("ehr-text");

my $options= $q->param("other_options"); # useless.
...
$ehr_text!~/^\W*$/ or $ehr_text!~/^\W*$/ or $ehr_file or die "No EHR input is detected!!!";
...
my $warning_message = '';
my $weblink;
my ($total_gene_num,$legal_gene_num)=(0,0);
...
#GenomicsServer::executeProgram ("$CGI_DIRECTORY/serve_ehr.pl -id $submission_id 2> $WORK_DIRECTORY/$submission_id/error_log");
if($warning_message eq '')
{ 
  GenomicsServer::executeProgram("perl $CGI_DIRECTORY/serve_ehr.pl 2> $WORK_DIRECTORY/$SUBMISSION_ID/serve_ehr_error_log") ;
}
...
sub writeInfoFile {
  ...
  open (BED, ">$orig_file") or confess "Error: cannot write query_bed file: $!";
    ...
    open (INFO, ">$WORK_DIRECTORY/$SUBMISSION_ID/info") or confess "Error: cannot write info file: $!";
        ...
        if (defined $ehr_file) {
          open (EHR, ">$WORK_DIRECTORY/$submission_id/ehr.txt") or die "Error: cannot write to user EHR file: $!\n";
          while (<$ehr_file_fh>) {
              print EHR;
          }
          close (EHR);
...
$replace_message.=$q->p ("You are using EHR-Phenolyzer mode.");
