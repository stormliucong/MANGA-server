...
sub processSubmission {
	...
	# This might cause an potential trouble.
	# For example, a task is lauched but interupted for some reason.
	# This task won't be processed again.
	...
	$system_command = "python $BIN_DIRECTORY/ehr_phenolyzer.py ";
	if($info{"ehr_file"}){$system_command.=" -i $info{ehr_file} ";}
	# $system_command.="-f ";
 ...
 #    }
	...
	# my $system_command_test = "python $BIN_DIRECTORY/sys_cmd_test.py ";
	# if($info{"ehr_file"}){$system_command_test.=" -i $info{ehr_file} ";}
	# $system_command_test .= " 2> query.disease_annotation.log";
...
my $rank=1;
	...
	# send email directly by system.
	if ($info{email}) {
		GenomicsServer::executeProgram("/usr/sbin/sendmail $info{email} < $WORK_DIRECTORY/$id/email") and warn '>' . scalar(localtime) . " (id: $id)\ncannot send mail\n" ;
	}
	...
	### comment this block for test purpose.
	##########################################################################################
...
=head1 SYNOPSIS
 ...
 Version: $LastChangedDate: 2018-01-22 08:53:41 -0700 (Tue, 21 May 2013) $
