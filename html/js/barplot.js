...
var BARPLOT = function (MAX){
	...
	function type(d) {
		...
		barsContainer.selectAll("rect").data(data).enter()
		  .append("rect")
		  .on('mouseover',function(d,i) { if(i<MAX) d3.select(this).attr('fill','#fecc33'); })
		  .on('mouseout',function(d,i) {  d3.select(this).attr('fill',function(d) { return colorScale(barValue(d)); }) ; })
		  .attr('data-toggle',"modal") 
		  .attr('data-target',function(d,i){ return "#gene"+i;})
		  .attr('y', y)
		  .attr('height', yScale.rangeBand())
		  .attr('width', function(d) { return x(barValue(d)); })
		  .attr('stroke', 'white')
		  .attr('fill', function(d) {return colorScale(barValue(d)); } )
		  .style('cursor',function(d,i){if(i<MAX) return 'pointer'; });
...
$('h3.result').after('<a class="outside" style="color:#010101;width:100%;padding-bottom:20px;display:block" href = "'+window.location.href+'out.final_gene_list">View source data</a>');
