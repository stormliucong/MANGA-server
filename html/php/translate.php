...
try {
	...
	$clientSecret = "1LJX703ItRPh/ZSzuzst+4G2JQfDo1iHv20r5Owpj+M=";
	//OAuth Url.
	...
	foreach ($texts as $each) {
		$getTranslationsurl = "http://api.microsofttranslator.com/V2/Ajax.svc/Translate?".
	                      "from=".urlencode($from).
	                      "&to=".urlencode($to).
	                      "&text=".urlencode($each);
		//Call the curlRequest.
		$curlResponse = $translatorObj->curlRequest($getTranslationsurl, $authHeader);
		$curlResponse = preg_replace('/^\W+(.*?)\W+$/','$1',$curlResponse);
		array_push($response, $curlResponse);
	}
	$response = join(";",$response);
	//Interprets a string of XML into an object.
	#$xmlObj = simplexml_load_string($curlResponse);
	echo json_encode(["translation" => $response]);
